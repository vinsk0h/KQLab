// investigations.js — KQLab Investigations module
...
function genReport(inv) {
  ...
  md += '---\n_Generated by **KQLab** on ' + new Date().toISOString().slice(0, 19).replace('T', ' ') + ' UTC_\n';
  return md;
}
...
function buildPreviewHTML(inv) {
  ...
  h += '<hr class="sep">';
  h += '<p style="font-size:11px;color:#9ca3af;text-align:center">Generated by KQLab · ' + new Date().toISOString().slice(0, 19).replace('T', ' ') + ' UTC</p>';
  return h;
}
